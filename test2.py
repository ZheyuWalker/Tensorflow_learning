...
2 pool layers
1 fully-connnected layer

1st convolution(stride=2): 4,4,8,32; bias:32
1st pool: 2 * 2
result: 6 * 8 * 32

2nd convolution(stride=1): 3,4,32,64; bias:32
2nd pool: 2 * 2
result: 3 * 4 * 64
...
'''
#Run the action, get the next state and reward
...
def createNetwork():
    # input layer
    s = tf.placeholder(tf.float32, [None, 24, 32, 8])

    # 1st convolutional layer and apply the Relu function
    W_conv1 = weight_variable([4, 4, 8, 32])
    ...
    W_conv2 = weight_variable([3, 4, 32, 64])
    b_conv2 = bias_variable([64])
    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2, 1) + b_conv2)
    # apply finally max pool
    h_pool2 = max_pool_2x2(h_conv2)

    # reshape to fit the fully connected layer
    h_pool2_flat = tf.reshape(h_pool2, [-1, 768])

    # 1st fully-connected layer and apply the Relu function
    W_fc1 = weight_variable([768, ACTIONS])
    b_fc1 = bias_variable([ACTIONS])
  
    # output layer
    readout = tf.matmul(h_pool2_flat, W_fc1) + b_fc1

    return s, readout, h_pool2_flat
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    x_t = cv2.cvtColor(cv2.resize(x_t, (32, 24)), cv2.COLOR_BGR2GRAY)
    ret, x_t = cv2.threshold(x_t, 1, 255, cv2.THRESH_BINARY)
    s_t = np.stack((x_t, x_t, x_t, x_t, x_t, x_t, x_t, x_t), axis = 2)
    ...
    while True:
        ...
        x_t1 = cv2.cvtColor(cv2.resize(x_t1_colored, (32, 24)), cv2.COLOR_BGR2GRAY)
        ret, x_t1 = cv2.threshold(x_t1, 1, 255, cv2.THRESH_BINARY)
        x_t1 = np.reshape(x_t1, (24, 32, 1))
        #s_t1 = np.append(x_t1, s_t[:, :, :7], axis = 2)
        s_t1 = np.append(x_t1, s_t[:, :, :7], axis = 2)
