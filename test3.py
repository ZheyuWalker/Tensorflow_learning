...
2 pool layers
2 fully-connnected layer

1st convolution(stride=2): 4,4,10,40; bias:40
1st pool: 2 * 2
result: 6 * 8 * 40

2nd convolution(stride=1): 3,4,40,80; bias:80
2nd pool: 2 * 2
result: 3 * 4 * 80
...
def createNetwork():
    # input layer
    s = tf.placeholder(tf.float32, [None, 32, 24, 10])

    # 1st convolutional layer and apply the Relu function
    W_conv1 = weight_variable([4, 4, 10, 40])
    ...
    W_conv2 = weight_variable([4, 3, 40, 80])
    ...
    h_pool2_flat = tf.reshape(h_pool2, [-1, 960])

    # 1st fully-connected layer and apply the Relu function
    W_fc1 = weight_variable([960, 480])
    b_fc1 = bias_variable([480])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

    # 2nd fully-connected layer and apply the Relu function
    W_fc2 = weight_variable([480, ACTIONS])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    x_t = cv2.cvtColor(cv2.resize(x_t, (24, 32)), cv2.COLOR_BGR2GRAY)
    ...
    while True:
        ...
        x_t1 = cv2.cvtColor(cv2.resize(x_t1_colored, (24, 32)), cv2.COLOR_BGR2GRAY)
        ret, x_t1 = cv2.threshold(x_t1, 1, 255, cv2.THRESH_BINARY)
        x_t1 = np.reshape(x_t1, (32, 24, 1))
